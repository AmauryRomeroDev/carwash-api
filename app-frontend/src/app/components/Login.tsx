...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  // FastAPI espera los datos en formato "Form Data" por el OAuth2PasswordRequestForm
  const formDataToSubmit = new FormData();
  formDataToSubmit.append('username', formData.email); // El backend usa data.username para el email
  formDataToSubmit.append('password', formData.password);

  try {
    const response = await fetch("http://localhost:8000/api/v1/auth/login/", {
      method: "POST",
      // No ponemos Content-Type manualmente, el navegador lo hará por el FormData
    ...
    if (response.ok) {
      // Guardamos el token y la info del usuario que devuelve tu API
      ...
      // Muestra "Credenciales inválidas" o el error que envíe FastAPI
